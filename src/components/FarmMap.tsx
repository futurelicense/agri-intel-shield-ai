...
const FarmMap: React.FC<FarmMapProps> = ({ location, onLocationChange }) => {
  ...
  // Simulate NDVI data based on location
  useEffect(() => {
    const simulateNdvi = () => {
      // Simulate varying NDVI values based on coordinates and time
      const baseNdvi = 0.6 + (Math.sin(location.lat * 0.1) * 0.2);
      const noise = (Math.random() - 0.5) * 0.1;
      setNdviData(Math.max(0, Math.min(1, baseNdvi + noise)));
    };
